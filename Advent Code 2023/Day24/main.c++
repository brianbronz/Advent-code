...
float isIntersect(struct feature * a, struct feature * b){
    float ax, ay, az, firstvx, avy, avz;
    float bx, by, bz, secondvx, secondvy, secondvz;
    ax = a->position[0];
    ay = a->position[1];
    az = a->position[2];
    firstvx = a->speed[0];
    ...
    secondvz = b->speed[2];
    //parametric
    float time = (secondvy * (bx - ax) - secondvx *(by - ay)) / (firstvx * secondvy - secondvx * avy);
    if(time < 0){return 0;}
    float space = (ax + firstvx * time - bx) / secondvx;
    if(space < 0){return 0;}
    float x = ax + firstvx * time;
    float y = ay + avy * time;
...
float determinant(float mat[6][6], int N){
    float det = 1;
    // Gauss elimination
    for (int i = 0; i < N; i++) {
...
float determinant(float mat[5][5], int N){
    float det = 1;
    // Gauss elimination
    for (int i = 0; i < N; i++) {
        //Highest value of the row
...
float subDeterminant(float mat[6][6], int row, int col) {
    float subMat[5][5];
    int subi = 0, subj = 0;
    ...
    float subDet = determinant(subMat, 6 - 1);
    return subDet;
}

float adjoint(float matrix[6][6], float cofactor[6][6]){
    for (int i = 0; i < 6; i++) {
        for (int j = 0; j < 6; j++) {
            int sign = ((i + j) % 2 == 0) ? 1 : -1;
            float subDet = subDeterminant(matrix, i, j);
            cofactor[j][i] = sign * subDet;
        }
    }
}

void calculateInverse(float matrix[6][6], float inverse[6][6]){
    //find the determinant
    float det = determinant(matrix, 6);
    //adjacent matrix
    float cofactor[6][6];
...
void multiplyMatrixVector(float mat[6][6], vector<float> vec, float result[6]) {
    for (int i = 0; i < 6; i++) {
        result[i] = 0;
        for (int j = 0; j < 6; j++) {
...
float secondPart(){
    ...
    vector<float> firstPos;
    vector<float> firstVel;
    ...
    vector<float> secondPos;
    vector<float> secondVel;
    ...
    vector<float> thirdPos;
    vector<float> thirdVel;
